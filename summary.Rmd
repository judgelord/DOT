...
There are `r dim(dotRIN)[1]` unique RINs for which we have the date at which they were initiated. Of these, `r sum(dotRIN$color == "Red", na.rm = T)` were marked 'delayed' and `r sum(dotRIN$color == "Green", na.rm = T)` were marked 'on time' at the time they were last observed. However, DOT increasingly marks projects as 'not on a schedule' ( `r sum(dotRIN$color == "Black", na.rm = T)` are last observed in the “Black” category). `r sum(dotMonthly$delayedever, na.rm = T)` projects were delayed and give a reason for delay at some point.
...
| MONTHS AFTER INITIATED:      | Sent to OST| Received by ORIA | Published | TOTAL PUBLISHED |
| ------------------ | ------------- | ---------------- | -------------------- | --------- |
...
# lawsuits against the subagency in the past x years
avg # of lawsuits against the agency per year of administration (this is perhaps better suited for a model that is not just DOT)
