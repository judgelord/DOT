...
There are `r dim(dotRIN)[1]` unique RINs for which we have the date at which they were initiated. Of these, `r sum(dotRIN$color == "Red", na.rm = T)` were marked 'delayed' and `r sum(dotRIN$color == "Green", na.rm = T)` were marked 'on time' at the time they were last observed. However, DOT increasingly marks projects as 'not on a schedule' ( `r sum(dotRIN$color == "Black", na.rm = T)` are last observed in the “Black” category). `r sum(dotMonthly$everdelayed, na.rm = T)` projects were delayed and give a reason for delay at some point.
`r sum(dotRIN$initiated > as.Date("2008-01-01"), na.rm = T)` Were initiated after 2008. Of these `r sum(grepl("Final Rule", dotRIN$STAGE), na.rm = T)` have Final Rules Published, `r sum(grepl("Withdrawal", dotRIN$STAGE), na.rm = T)`  have been withdrawn, and `r sum(grepl("Termination", dotRIN$STAGE), na.rm = T)` were terminated.
 ...
 | Days Until OST                    | Received by ORIA | Days Until Published | Published |
 | --------------------------------- | ---------------- | -------------------- | --------- |
ANPRMs              | ||||
NPRMs               |||||
SNPRMs              |||||
